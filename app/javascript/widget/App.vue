...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.channelXWebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.channelXWebChannel;
    ...
    registerCampaignEvents() {
      ...
      emitter.on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.channelXWebChannel;
