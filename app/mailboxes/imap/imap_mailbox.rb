class Imap::ImapMailbox
  ...
  def find_conversation_by_microsoft_thread_id
    return if microsoft_thread_id.blank?

    @inbox.conversations.find_by("additional_attributes->>'microsoft_thread_id' = ?", microsoft_thread_id)
  end

  # Outlook's conversationId, stamped by Imap::MicrosoftFetchEmailService.
  def microsoft_thread_id
    @inbound_mail['X-MS-Thread-ID']&.value
  end
  ...
  def find_or_create_conversation
    @conversation = find_conversation_by_gmail_thread_id || find_conversation_by_microsoft_thread_id ||
                    find_conversation_by_in_reply_to || find_conversation_by_reference_ids || ::Conversation.create!(
                      {
                        ...
                        additional_attributes: {
                          ...
                          microsoft_thread_id: microsoft_thread_id,
    ...
    backfill_thread_id(:gmail_thread_id, gmail_thread_id)
    backfill_thread_id(:microsoft_thread_id, microsoft_thread_id)
  end

  # A conversation matched by in-reply-to/references may predate the provider thread id
  # (e.g. it was started by an agent reply). Stamp it so later messages match on it directly.
  def backfill_thread_id(key, value)
    return if @conversation.blank? || value.blank?
    return if @conversation.additional_attributes[key.to_s].present?

    @conversation.additional_attributes = @conversation.additional_attributes.merge(key => value)
    @conversation.save!
  end
